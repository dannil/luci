...
return view.extend({
	...
	load: function() {
		...
		return Promise.all([load_squid, getOptions()]);
	},
	...
	render: function(data) {
		...
		o = s.taboption('general', form.Flag, 'pinger_enable', _('Enable ICMP pinger'),
			!is_enable_icmp_defined ? _('Can only be set if Squid is compiled with the %s option').format(enable_icmp_option) : null);
		o.datatype = 'string';
		o.enabled = 'on';
		o.disabled = 'off';
